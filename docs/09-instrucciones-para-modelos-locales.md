...
| Empezar sin comprobar si ya está hecho | Escribí 400 líneas de `data/detectores.toml` y los 18 detectores existían desde `7b995a3`, con la curva de D10 ya resuelta. Segunda vez en la misma sesión: antes lancé agentes sobre FG-07 y FG-08 estando terminadas | `git pull`, y un `grep` del entregable en `data/`, `dlv-core/src` y las pruebas ANTES de escribir. El estado puede estar mal; el repositorio no |
